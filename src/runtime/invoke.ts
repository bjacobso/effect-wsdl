...
export function validateClientConfig(endpoint: string, config: ClientConfig): URL {
  const bytes = config.maxResponseBytes ?? 10 * 1024 * 1024;
  const timeout = config.timeoutMs ?? 30000;
  if (!Number.isSafeInteger(bytes) || bytes < 1 || !Number.isFinite(timeout) || timeout <= 0)
    throw new Error("Invalid timeout or response byte limit");
...
export function invoke<A, B, D>(
  ...
  return Effect.suspend(() => {
    ...
    const request = Effect.try({
      try: () => {
        const url = validateClientConfig(endpoint, config);
